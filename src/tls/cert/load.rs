...
pub static LOAD_CERTS: LazyLock<Option<RootCertStore>> = LazyLock::new(|| {
    ...
    let res = RootCertStore::from_der_certificates(webpki_root_certs::TLS_SERVER_ROOT_CERTS);

    #[cfg(all(feature = "native-roots", not(feature = "webpki-roots")))]
    let res = RootCertStore::from_der_certificates(rustls_native_certs::load_native_certs().certs);
