...
impl<'a> CertificateInput<'a> {
    pub(crate) fn with_parser<F>(self, parser: F) -> Result<Certificate, Error>
    where
        F: Fn(&'a [u8]) -> Result<Certificate, Error>,
    {
        match self {
...
impl Certificate {
    ...
    pub fn from<C: AsRef<[u8]>>(cert: C) -> Result<Self, Error> {
