...
/// An iterator over query parameters in a query string.
pub struct QueryPairs<'a>(&'a str, Enumerate<Chars<'a>>);
...
/// Parse a query string into an iterator of QueryParam.
pub fn parse_query_params(query_string: &str) -> impl Iterator<Item = QueryParam<'_>> {
    assert!(query_string.is_ascii());
    QueryPairs(query_string, query_string.chars().enumerate())
}

/// Percent-encode a string using the ENCODED_IN_QUERY set.
pub fn url_enc<'a>(i: &'a str) -> Cow<'a, str> {
...
pub fn form_url_enc<'a>(i: &'a str) -> Cow<'a, str> {
...
mod test {
    ...
    fn parse_query_string() {
        assert!(parse_query_params("").next() == None);
        assert_eq!(p("&"), vec![""]);
        assert_eq!(p("="), vec!["="]);
        assert_eq!(p("&="), vec!["", "="]);
